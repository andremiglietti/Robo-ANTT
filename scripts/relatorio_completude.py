...
def main() -> None:
    ...
    print(f"Confirmadas 100% completas: {len(confirmadas)} ({100 * len(confirmadas) / total_esperado:.1f}%)")

    if not faltando:
        print("\n[OK] TODAS as combinações CNPJ×tipo da empresa estão confirmadas 100% completas.")
        return

    print(f"\n[ATENÇÃO] Faltam {len(faltando)} combinação(ões) CNPJ×tipo - agrupadas por CNPJ:")
