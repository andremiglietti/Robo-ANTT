...
**Ou seja: os Blocos 1–3 do checklist, que eram bloqueantes para começar a escrever código, estão todos fechados, e a varredura completa (login/sessão, CNPJs, busca por todos os tipos, paginação) está escrita e validada ao vivo de ponta a ponta** — inclusive corrigindo 4 bugs reais que só apareceram testando contra o portal de verdade (hack do `chosen.js`, busca sem filtro de tipo travando, paginador não confiável, modal "Processando..." bloqueando cliques). Próximo passo: download do PDF e extração de campos.
...
3. Escrever o download do PDF (clique na lupa + `page.expect_download()`) e a extração de campos da página 1/boleto — próximos blocos de código (Dias 8–9 do cronograma).
4. Confirmar os itens pendentes do Bloco 6 (permissão de escrita no SharePoint, aprovação de InfoSec, ponto de contato do TI) — em paralelo, não bloqueia o código.
