...
5. **Download e extração de PDF**: baixar o PDF do auto e extrair campos estruturados. O layout pode variar por tipo de multa (frete mínimo, pedágio, excesso de peso, produtos perigosos).
   ...
   - ⚠️ **Ainda pendente:** os campos que ficam numa página **posterior** à página 1 (boleto/notificação de penalidade) — `Data Emissão Notificação`, `Data Emissão Boleto`, `Data Vencimento`, `Valor`, `Valor Desconto`, `Código de Barras`. Diferente da página 1 (sempre no mesmo lugar), essa página varia de posição dentro do PDF (era a página 7 no exemplo de 19 páginas) — a extração vai precisar **procurar** a página certa (ex.: por conter "VALOR TOTAL DA MULTA" ou similar) em vez de assumir um número fixo, o que pode ser lento em PDFs de até 79 páginas.
...
- `src/robo_antt/extracao.py` — extração de texto/campos do PDF (`pdfplumber`): tipo de multa (cabeçalho) e os campos da página 1 (placa, CNPJ, data, descrição, documento fiscal), tolerante ao bug de acentuação e às variações de rótulo entre os 5 tipos de multa. **Validado: tipo de multa 5/5, campos da página 1 35/35.**
...
**Ou seja: os Blocos 1–3 do checklist estão fechados, e a varredura + download do PDF + identificação do tipo de multa estão escritos e validados ao vivo de ponta a ponta** — inclusive corrigindo 5 bugs reais que só apareceram testando contra o portal/PDFs de verdade (hack do `chosen.js`, busca sem filtro de tipo travando, paginador não confiável, modal "Processando..." bloqueando cliques, tempo de resposta variável demais para 1 tentativa só). Falta a extração dos demais campos da planilha (datas, valor, descrição, etc.) pra fechar o Dia 9.
...
4. ~~Extrair os campos da página 1 do PDF~~ — feito e validado em 16/09/2026 (35/35 campos corretos nos 5 exemplos).
5. Extrair os campos que ficam numa página posterior (boleto/notificação): Data Emissão Notificação, Data Emissão Boleto, Data Vencimento, Valor, Valor Desconto, Código de Barras — precisa localizar a página certa dentro do PDF (não é fixa) antes de extrair. Pode ser feito offline com os 5 PDFs de exemplo.
6. Confirmar os itens pendentes do Bloco 6 (permissão de escrita no SharePoint, aprovação de InfoSec, ponto de contato do TI) — em paralelo, não bloqueia o código.
